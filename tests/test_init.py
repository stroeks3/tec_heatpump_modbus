...
async def test_switch_write_failure_raises(hass: HomeAssistant, mock_modbus_client) -> None:
    ...
    from custom_components.tec_heatpump_modbus.const import SWITCHES
    from custom_components.tec_heatpump_modbus.switch import TECHeatPumpSwitch

    entry = await _setup_entry(hass)
    coordinator = entry.runtime_data
    switch = TECHeatPumpSwitch(coordinator, SWITCHES[0])
